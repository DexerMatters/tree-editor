    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.matchers.len() <= 4 {
            ...
            if let Some((l, id)) = best {
                ...
                let node = self
                    .alloc
                    .new_token(id, text.to_string(), None, self.prev.clone());
                // link as doubly-linked list: set prev.right to this node (once), and set this.left to prev
                if let Some(prev_node) = &self.prev {
                    match Arc::as_ref(prev_node) {
                        Tree::Token { right, .. } | Tree::Hole { right, .. } => {
                            let mut guard = right.lock().unwrap();
            ...
            // No matcher matched; produce a single-char Hole
            let ch = remaining.chars().next().unwrap();
            self.pos += ch.len_utf8();
            let node = self.alloc.new_hole(
                ch.to_string(),
                HoleType::Incomplete,
                None,
                self.prev.clone(),
        ...
        // No matcher succeeded: produce Hole
        let ch = remaining.chars().next().unwrap();
        self.pos += ch.len_utf8();
        let node = self.alloc.new_hole(
            ch.to_string(),
...
mod test {
    ...
    fn test_lexer() {
        ...
        let mut lexer = LexIter::new(alloc, &grammar, text);
        while let Some(token) = lexer.next() {
            println!("Token green id: {:?}", token); // debug the produced Tree node
        }
    }
}
...
impl<'a> PartialParser<'a> {
    pub fn new(alloc: TreeAlloc, grammar: &'a Grammar, path: Vec<TreeId>) -> Self {
        Self {
            alloc,
            grammar,
            path,
